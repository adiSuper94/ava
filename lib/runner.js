...
export default class Runner extends Emittery {
	constructor(options = {}) {
		...
		this.chain = createChain((metadata, testArgs) => { // eslint-disable-line complexity
			...
			always: false,
		}, meta, (tailMetaData, tailArgs) =>{
			// if(!tailMetaData.failing){
			// 	return;
			// }
			const reason = tailArgs[0];
			if (typeof reason !== 'string') {
				throw new TypeError('The reason for failing a test must be a string');
			}
			console.log(`reason of failuer is ${reason}`);
			this.emit('stateChange', {
				type: 'reason-called',
				title: "reason",
				knownFailing: true,
