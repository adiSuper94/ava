...
function startChain(name, call, defaults, tailChain) {
	const fn = (...args) => {
		call({...defaults}, args);
		if(tailChain) {
			let tilte = args[0];
			tailChain.title = tilte;
			return tailChain;
		}
	};
...
export default function createChain(fn, defaults, meta, tailFn) {
	// Test chaining rules:
	...
	// * no repeating
	const tail = startChain('tail', tailFn, {...defaults, type: 'tail'});
	extendChain(tail, 'because');
	const root = startChain('test', fn, {...defaults, type: 'test'}, tail);
